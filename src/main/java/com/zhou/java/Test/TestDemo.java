...
//public class TestDemo<K,V> extends AbstractMap<K,V>
...
//
//    // 数组扩容： 容量扩大为指定数(2的n次数)
//    // 假设原来数组长度为16，则在调用tryPresize的时候，size参数的值为16<<1(32)，此时sizeCtl(临界阈值)为12
//    // 计算出c的值为64，则要扩容到sizeCtl > 为止
//    //    第一次扩容后，数组长度为：32  sizeCtl:24
//    //    第二次扩容后，数组长度为：64  sizeCtl:48
//    //    第三次扩容后，数组长度为：128  sizeCtl:94  --> 这时候才会退出扩容
//    private final void tryPresize(int size) {
//        // MAXIMUM_CAPACITY = 1 << 30
//        // 给定的容量大小等于数组容量的一半，则直接使用最大容量。否则用tableSizeFor计算
//        // 后面table一直扩容到这个值小于等于sizeCtl（数组长度的3/4）才退出扩容
//        int c = (size >= (MAXIMUM_CAPACITY >>> 1)) ? MAXIMUM_CAPACITY :
//                tableSizeFor(size + (size >>> 1) + 1);
//        int sc;
//        while ((sc = sizeCtl) >= 0) {
//            // 若数组table没有被初始化，则初始化一个大小为sizeCtl和c中较大的一个大小的数组
//            // 初始化时，sizeCtl设置为-1，初始化完成后sizeCtl = 数组长度的3/4
//            // 为何在扩张时有初始化数组操作？因为第一次put时不是单个元素，而是putAll一个map的话，putAll中没有调用initTable初始化数组
//            // 而是调用tryPresize方法，所以在该方法里要判断是否需要初始化table
//            Node<K,V>[] tab = table; int n;
//            if (tab == null || (n = tab.length) == 0) {
//                n = (sc > c) ? sc : c;
//                if (U.compareAndSwapInt(this, SIZECTL, sc, -1)) { //调用unSafe方法，硬件原子操作。初始化tab时，把sizeCtl设为-1
//                    try {
//                        if (table == tab) {
//                            @SuppressWarnings("unchecked")
//                            Node<K,V>[] nt = (Node<K,V>[])new Node<?,?>[n];
...
//                }
//            }
//            // 一直扩容到c 小于等于sizeCtl或者数组长度大于最大长度时则推送，不再扩容
//            // 所以在扩容后，新长度不是原来长度的两倍，而是2的n次方倍
...
//                            transferIndex <= 0)
//                        break;
//                    // transfer的线程数加一，该线程将进行transfer的帮忙，在transfer的时候，sc表示在transfer工作的线程数
//                    if (U.compareAndSwapInt(this, SIZECTL, sc, sc + 1))
//                        transfer(tab, nt);
//                }
//                // 没有再初始化或扩容，则开始扩容
